...
Границы модуля:
  - Всё, что говорит с BingX API (создание/отмена/чтение ордеров, позиций,
    контрактов) — здесь.
  - Всё, что решает ЧТО логировать в execution_events.jsonl, отправлять в
    Telegram, как менять watchlist/lifecycle_state — в monitor.py.
  - Функции здесь по возможности не имеют побочных эффектов, кроме записи
    в bingx_orders.jsonl (низкоуровневый техжурнал самого клиента) и вызовов
    к самой бирже.

TP и SL — исключительно execution/risk-management слой.
