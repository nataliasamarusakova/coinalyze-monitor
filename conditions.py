...
# РЕЖИМ ВНЕДРЕНИЯ
#
# Изменения СИГНАЛЬНОЙ логики идут только через эксперимент: сначала теневой
# расчёт, потом сравнение на накопленной выборке, потом боевое включение с
# инкрементом SIGNAL_LOGIC_VERSION. Поэтому И1 и И3 выключены — они считаются
# каждый прогон и пишутся в shadow_signals.jsonl, но на вердикт не влияют.
#
# Боевым остаётся только И5 (трёхзначная логика), и то потому, что на текущих
# данных она инертна: у cvd24, oi_chg24_pct, price_chg24, oi_chg4h_pct, lls24
# и fr_oiw пропусков 0%. Она меняет вердикт лишь при сбое парсинга, где старое
# поведение (safe(None)→0.0 → «максимально медвежий») было заведомо неверным.
# ═══════════════════════════════════════════════════════════════════════════

# ─── И1: k-of-n ─── ТЕНЕВОЙ РЕЖИМ ───────────────────────────────────────────
USE_KOFN = False
KOFN_K = 4                      # сколько снимков из CONFIRMED_A_SNAPS должны пройти порог

# ─── И3: гейт плотности окна ─── ТЕНЕВОЙ РЕЖИМ ──────────────────────────────
...
# ─── И2: итоговое изменение вместо шаговой монотонности ─── ТЕНЕВОЙ ─────────
USE_NET_CHANGE = False
...
CONFIG = {
    ...
    "live_verdict_equals": "v1 (all-of-n, без гейта плотности)",
}
