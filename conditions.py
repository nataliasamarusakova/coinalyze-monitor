...
CONFIRMED_A_OI4H_POS = True
CONFIRMED_A_FR_MAX = 0.05
CONFIRMED_A_LLS_MAX = 40.0
CONFIRMED_A_PC_TOLERANCE = 0.5
CONFIRMED_A_OI_TOLERANCE = 1.0
CONFIRMED_A_CVD_TOLERANCE = 5.0
...
def check_confirmed_path_a(snaps: list[dict]) -> dict:
    ...
    conds["oi_chg24_gt5"] = {
        "met": met_oi, "value": round(worst_oi, 2),
    ...
    conds["cvd_gt55"] = {
        "met": met_cvd, "value": round(worst_cvd, 2),
    ...
    conds["oi4h_positive"] = {
        "met": met_oi4, "value": round(min(vals_oi4), 3), "threshold": 0,
...
def check_confirmed_path_b(snaps: list[dict], cvd_momentum: float) -> dict:
    ...
    vals_cvd = [safe(s.get("cvd24")) for s in recent]
    met_cvd = all(v > CONFIRMED_B_CVD_MIN for v in vals_cvd)
    worst_cvd = min(vals_cvd)
    conds["cvd_gt50"] = {
        "met": met_cvd, "value": round(worst_cvd, 2),
    ...
    conds["cvd_momentum_gt5"] = {
        "met": met_mom, "value": round(cvd_momentum, 2),
    ...
    # Общие условия (как в path_a) — FIX C2
    vals_oi4 = [safe(s.get("oi_chg4h_pct")) for s in recent]
    met_oi4 = all(v > 0 for v in vals_oi4)
    conds["oi4h_positive"] = {
        "met": met_oi4, "value": round(min(vals_oi4), 3), "threshold": 0,
...
def check_early_move(snaps: list[dict]) -> dict:
    if len(snaps) < EARLY_MOVE_SNAPS:
        return {"passed": False, "insufficient_data": True,
                ...
                "conditions": {}}
    last3 = snaps[-EARLY_MOVE_SNAPS:]
    conds = {}
    price_up = all(safe(last3[i].get("price_chg24")) > safe(last3[i-1].get("price_chg24")) for i in range(1, EARLY_MOVE_SNAPS))
    conds["price_up"] = {"met": price_up, "value": None, "threshold": None, "deficit": 0, "met_count": None, "total": None}
    oi_up = all(safe(last3[i].get("oi_chg24_pct")) > safe(last3[i-1].get("oi_chg24_pct")) for i in range(1, EARLY_MOVE_SNAPS))
    conds["oi_up"] = {"met": oi_up, "value": None, "threshold": None, "deficit": 0, "met_count": None, "total": None}
    cvd_up = all(safe(last3[i].get("cvd24")) > safe(last3[i-1].get("cvd24")) - EARLY_MOVE_CVD_TOLERANCE for i in range(1, EARLY_MOVE_SNAPS))
    conds["cvd_up"] = {"met": cvd_up, "value": None, "threshold": None, "deficit": 0, "met_count": None, "total": None}
    vol_up = all(safe(last3[i].get("volume24")) > safe(last3[i-1].get("volume24")) * EARLY_MOVE_VOL_FACTOR for i in range(1, EARLY_MOVE_SNAPS))
    conds["vol_up"] = {"met": vol_up, "value": None, "threshold": None, "deficit": 0, "met_count": None, "total": None}
    passed = all(c["met"] for c in conds.values())
    return {"passed": passed, "insufficient_data": False, "conditions": conds}
...
def check_accumulation(snaps: list[dict]) -> dict:
    if len(snaps) < ACCUMULATION_SNAPS:
        return {"passed": False, "insufficient_data": True,
                ...
                "conditions": {}}
    last3 = snaps[-ACCUMULATION_SNAPS:]
    conds = {}
    oi4_pos = all(safe(s.get("oi_chg4h_pct")) > 0 for s in last3)
    conds["oi4h_positive"] = {"met": oi4_pos, "value": None, "threshold": None, "deficit": 0, "met_count": None, "total": None}
    cvd_avg = sum(safe(s.get("cvd24")) for s in last3) / ACCUMULATION_SNAPS
    met_cvd = cvd_avg > ACCUMULATION_CVD_AVG_MIN
    conds["cvd_avg_gt50"] = {"met": met_cvd, "value": round(cvd_avg, 2), "threshold": ACCUMULATION_CVD_AVG_MIN,
                             ...
                             "met_count": None, "total": None}
    pc = safe(last3[-1].get("price_chg24"))
    ...
    conds["price_chg_lt5"] = {"met": met_pc, "value": round(pc, 2), "threshold": ACCUMULATION_PC_MAX,
                              ...
                              "met_count": None, "total": None}
    fr_ok = all(s.get("fr_oiw") is not None and s.get("fr_oiw") < ACCUMULATION_FR_MAX for s in last3)
    conds["fr_lt003"] = {"met": fr_ok, "value": None, "threshold": None, "deficit": 0, "met_count": None, "total": None}
    passed = all(c["met"] for c in conds.values())
    return {"passed": passed, "insufficient_data": False, "conditions": conds}


# ═══════════════════════════════════════════════════════════
# УТИЛИТЫ ДЛЯ FAIL_POINT
# ═══════════════════════════════════════════════════════════
...
def closest_miss_for_confirmed(snaps: list[dict], cvd_momentum: float) -> dict:
    ...
    if best_a is None and best_b is None:
        return {"path": None, "condition": None, "deficit": 0, "note": "оба пути пройдены"}
    if best_a is None:
...
def _closest_fail(result: dict) -> Optional[dict]:
    ...
    if result.get("insufficient_data"):
        return {"condition": "insufficient_data", "deficit": float("inf"), "value": None, "threshold": None}
    ...
    if numeric_fails:
        numeric_fails.sort(key=lambda x: x[1]["deficit"])
        name, c = numeric_fails[0]
