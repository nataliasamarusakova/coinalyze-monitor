...
в pending_trades.jsonl и ждут наступления горизонтов, затем финализируются.

Пороги входа НЕ калибруем по малой выборке — копим 200-300 сделок, потом
двигаем по срезам trades_report.py.

Запуск: python monitor.py  (cron-job.org → repository_dispatch → GitHub Actions)
"""
...
MARKET_TTL_DAYS    = 2          # [NEW] было 1; нужно для movement_snaps упущенных движений
...
def calc_derived(snaps: list[dict]) -> dict:
    n = len(snaps)
    d = {
        "oi_accel": 0.0, "cvd_momentum": 0.0, "price_accel": 0.0,
        "funding_pressure": 0.0,
        "oi_trend": "flat", "cvd_trend": "flat", "price_trend": "flat",
        "oi4h_trend": "flat", "divergence": "none", "note": "",
    }
    if n < 2:
        return d
...
def fetch_btc_price_chg() -> Optional[float]:
...
ACTIONS = {
    "NEUTRAL": "IGNORE", "ACCUMULATION": "WATCH", "EARLY_MOVE": "WATCH_LONG",
    "CONFIRMED_TREND": "POSSIBLE_ENTRY", "ACCELERATION": "LONG_SETUP",
    "EXHAUSTION": "NO_NEW_ENTRY", "DISTRIBUTION": "EXIT_AVOID",
    "INVALIDATED": "REMOVE",
}
STATE_EMOJI = {
...
def detect_lifecycle(symbol, snaps, score, derived, prev_state="NEUTRAL"):
    ...
    if allowed("ACCELERATION"):
        if (n >= 3 and oi_accel > 2 and cvd_mom > 10 and price_accel >= 1
                and derived["oi_trend"] != "down" and derived["cvd_trend"] != "down"):
    ...
    if allowed("CONFIRMED_TREND") and n >= MIN_SNAPS_LIFECYCLE:
        ...
        oi_not_falling = all(
            safe(recent[i].get("oi_chg24_pct")) >= safe(recent[i-1].get("oi_chg24_pct")) - 1
            for i in range(1, len(recent)))
        cvd_not_falling = all(
            safe(recent[i].get("cvd24")) >= safe(recent[i-1].get("cvd24")) - 5
            for i in range(1, len(recent)))
        ...
        all_pc  = all(
            safe(recent[i].get("price_chg24")) >= safe(recent[i-1].get("price_chg24")) - 0.5
            for i in range(1, len(recent)))
        ...
        if len(recent) >= 3:
            ...
            d1, d2 = ov[1] - ov[0], ov[2] - ov[1]
            oi_growing_faster = d2 > d1 and d1 > 0
        path_a = (all(safe(s.get("oi_chg24_pct")) > 5 for s in recent)
                  and all(safe(s.get("cvd24")) > 55 for s in recent))
        path_b = (all(safe(s.get("oi_chg24_pct")) > 2 for s in recent)
                  and all(safe(s.get("cvd24")) > 50 for s in recent)
                  and oi_growing_faster and cvd_mom > 5)
        trends_ok = derived["cvd_trend"] != "down" and derived["oi_trend"] != "down"
        if ((path_a or path_b) and all_oi4 and all_fr and all_lls
                and all_pc and pc_net_up and oi_not_falling and cvd_not_falling and trends_ok):
            is_early = path_b and not path_a
            if is_early:
                reasons.append("Раннее подтверждение: OI>2 CVD>50 + ускорение + momentum")
            else:
                reasons.append(f"{MIN_SNAPS_LIFECYCLE} снимков: OI>5 CVD>55 LLS<40 OI4h>0 P↑ FR<0.05")
...
# ═══════════════════════════════════════════════════════════
# 10-11. CONFIDENCE + EARLINESS
# ═══════════════════════════════════════════════════════════

def calc_confidence(state, snaps, score, derived, market_mod):
    base = {"NEUTRAL": 50, "ACCUMULATION": 40, "EARLY_MOVE": 55,
            "CONFIRMED_TREND": 70, "ACCELERATION": 80,
            "EXHAUSTION": 75, "DISTRIBUTION": 70, "INVALIDATED": 90}.get(state, 50)
...
def send_tg(text: str):
    ...
    for ch in chunks:
        try:
            ...
            if r.status_code != 200:
                requests.post(url, data={"chat_id": TG_CHAT_ID, "text": ch,
...
def llm_verify(symbol, wl, cur, snaps):
    ...
    snap_txt = "\n".join(
        ...
        f"FR={s.get('fr_oiw')}" for s in recent)
    user_msg = (
        f"Монета: {symbol}\nState: {state}\nConfidence: {conf}%\nAction: {action}\n"
    ...
    try:
        resp = requests.post(
            ...
            json={"model": QWEN_MODEL,
                  "messages": [{"role": "system", "content": "Верификатор крипто-сигналов. Только JSON."},
...
def open_trade_record(r, ts, state, path, score, momentum, conf,
                      early_val, early_label, pattern, derived, market,
                      first_seen, snapshots, price):
    ot = {
        "entry_ts": ts, "entry_price": price, "entry_state": state,
        "entry_path": path, "last_price": price, "last_price_ts": ts,
...
def close_trade(ot, symbol, exit_ts, exit_price, exit_reason, exit_state,
                price_full, exit_price_source="live"):
    ep = ot.get("entry_price")
...
def flush_pending(price_full, now, existing_trade_ids):
    ...
    for item in PENDING:
        ...
        ready = all(
            ...
            for h in TRADE_HORIZONS) or now >= ets + wait_max
        if ready:
...
def run():
    ...
    for r in rows:
        append_jsonl(HEARTBEAT_FILE, {"ts": ts, "symbol": r["symbol"],
                                      "price": r.get("price")})
        sym = r["symbol"]
        lifecycle_state = wl_all.get(sym, {}).get("state")
    ...
    for sym, hist in history_all.items():
                ...
                if old in ACTIVE_STATES:
                    ...
                    if nr >= NEUTRAL_HYSTERESIS:
                        ...
                        if ot:
                            ...
                            close_trade(ot, sym, ts, exit_price, "NEUTRAL", old,
                                        price_full, exit_price_source=src)
                        send_tg(f"⚪ <b>{esc(wl_all[sym].get('name', sym))} ({esc(sym)})</b>\n"
                                f"━━━━━━━━━━━━━━━━━━\n{esc(old)} → NEUTRAL\n"
                                f"<i>Снята: условия тренда не выполняются {nr} прогона подряд</i>\n")
        ...
        if state == "INVALIDATED":
            if sym in wl_all:
                ...
                if ot:
                    exit_price = cur_price or ot.get("last_price")
                    src = "live" if cur_price else "last_seen"
                    close_trade(ot, sym, ts, exit_price, "INVALIDATED", state,
                                price_full, exit_price_source=src)
                send_tg(f"❌ <b>{esc(wl_all[sym].get('name', sym))} ({esc(sym)})</b>\n"
                        f"━━━━━━━━━━━━━━━━━━\n{esc(old)} → INVALIDATED\n"
        ...
        if ot is not None:
            ...
            if close_reason:
                exit_price = cur_price or ot.get("last_price")
                src = "live" if cur_price else "last_seen"
                close_trade(ot, sym, ts, exit_price, close_reason, state,
                            price_full, exit_price_source=src)
            else:
                ot = dict(ot)
                if cur_price:
                    ...
                    if ep:
                        ...
                        if pnl_now > ot.get("max_pnl_pct", 0.0):
                            ot["max_pnl_pct"] = pnl_now; ot["peak_ts"] = ts
    ...
    for sym in list(wl_all.keys()):
        ...
        if missed >= MISS_REMOVE_RUNS:
            ot = wl_all[sym].get("open_trade")
            if ot:
                close_trade(ot, sym, ts, ot.get("last_price"), "MISSED", stage,
                            price_full, exit_price_source="last_seen")
