Вот полностью готовый и рабочий Python-код. 

В класс `QwenAPIClient` (который заменил тяжелый Playwright) добавлена **точная фильтрация фаз ответа**. Теперь скрипт игнорирует внутренние "размышления" нейросети (`phase: "thinking_summary"`) и забирает только чистый финальный текст (`phase: "answer"`), который затем отправляется в Telegram.
