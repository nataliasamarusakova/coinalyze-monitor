...
PARTIAL_TP_LEVELS = [
    {"leg": "tp1", "min_pnl_pct": 4.0, "close_fraction": 0.15},
    {"leg": "tp2", "min_pnl_pct": 7.0, "close_fraction": 0.25},
]

PARTIAL_MIN_QTY = 0.001
PARTIAL_MIN_NOTIONAL = 1.0
...
RECONCILE_AUTOCLOSE = (
    os.environ.get("BINGX_RECONCILE_AUTOCLOSE", "false").lower() == "true"
)
...
def classify_asset_class(r):
    ...
    if sym in EQUITY_SYMBOLS or any(h in name for h in EQUITY_HINTS):
        return "equity"
    if (
        sym in BTC_SYMBOLS or sym
    ):  # известно, что это скрапится с крипто-биржи — трактуем неизвестный тикер как крипто ТОЛЬКО если символ вообще есть
        pass
    return (
        "unknown"
        if not sym
        ...
        and sym not in COMMODITY_SYMBOLS
        and sym is not None
        and False
        else "crypto"
    )
...
def classify_asset_class(
    r,
):  # noqa: F811 — финальное, единственное действующее определение
...
def log_execution_event(symbol, kind, **fields):
    ...
    try:
        append_jsonl(
            ...
            {"ts": now_ts(), "symbol": symbol, "kind": kind, **fields},
        )
    except Exception as e:
...
def open_trade_record(
    ...
    entry_fr_oiw_zscore = calc_entry_fr_oiw_zscore_from_hist(hist, research_fr_oiw)
...
def _attempt_exchange_close(ot, symbol):
    ...
    if status == "closed":
        ...
        ot["bingx"] = bx
        log.info(
            f"[{symbol}] BingX CLOSE ok orderId={res.get('order_id')} qty={qty_to_close}"
        )
        log_execution_event(
            symbol, "close_confirmed", qty=qty_to_close, order_id=res.get("order_id")
        )
        return "confirmed"
    log.error(
        f"[{symbol}] BingX CLOSE НЕ подтверждён: status={status} error={res.get('error')}"
    )
    ot.setdefault("close_attempts", []).append(
        {"ts": now_ts(), "status": status, "error": str(res.get("error"))[:200]}
    )
    send_tg(
        f"🚨 <b>{esc(ot.get('name', symbol))} ({esc(symbol)})</b> — закрытие НЕ подтверждено биржей\n"
...
def _execution_reduce_to_fraction(ot, symbol, cur_price, target_fraction, tag, note):
    ...
    try:
        ...
        if real_amt < qty_to_close:
            log.warning(
                f"[{symbol}] EXEC_REDUCE({tag}) skipped: real_amt={real_amt:.8f} < qty_to_close={qty_to_close:.8f}"
            )
            return False
        res = bingx_client.close_long(symbol, qty_to_close)
    except Exception as e:
    ...
    log.info(
        f"[{symbol}] EXEC_REDUCE({tag}): closed {closed_qty:.8f} @ {cur_price} remaining={qty_remaining:.8f} ({remaining_pct:.1f}%)"
    )
    log_execution_event(
        symbol,
        f"reduce_{tag}",
    ...
    send_tg(
        f"🛡 <b>{esc(ot.get('name', symbol))} ({esc(symbol)})</b> — {esc(note)}\n"
        f"━━━━━━━━━━━━━━━━━━\n"
...
def _step_partial_take_profit(ot, symbol, cur_price):
    ...
    for level in PARTIAL_TP_LEVELS:
        ...
        try:
            ...
            if real_amt < qty_to_close:
                log.warning(
                    f"[{symbol}] PARTIAL_TP {leg_name} skipped: real_amt={real_amt:.8f} < qty_to_close={qty_to_close:.8f}"
                )
                continue
            res = bingx_client.close_long(symbol, qty_to_close)
        except Exception as e:
        ...
        if res.get("status") == "closed":
            ...
            remaining_pct = qty_remaining / qty_initial * 100 if qty_initial else 0.0
            log.info(
                f"[{symbol}] PARTIAL_TP {leg_name}: closed {closed_qty:.8f} @ {cur_price} pnl={pnl_pct:+.2f}% remaining={qty_remaining:.8f}"
            )
            log_execution_event(
                symbol,
                f"partial_tp_{leg_name}",
                closed_qty=closed_qty,
                price=cur_price,
            ...
            log.info(
                f"[{symbol}] PARTIAL_TP {leg_name} skipped by exchange: {res.get('error')}"
            )
        else:
...
def _step_break_even(ot, symbol, cur_price):
    ...
    if cur_price > trigger_price:
        return False
    ok = _execution_reduce_to_fraction(
        ot,
        symbol,
        cur_price,
        BREAK_EVEN_REDUCE_TO_FRACTION,
        tag="break_even",
        note="Break Even — защита капитала",
    )
    if ok:
        be["done"] = True
...
def _step_trailing_stop(ot, symbol, cur_price):
    ...
    ex = ot.setdefault("execution", {})
    tr = ex.setdefault(
        "trailing", {"active": False, "peak_price": None, "steps_done": 0}
    )
    pnl_pct = (cur_price - entry_price) / entry_price * 100
    if not tr["active"]:
    ...
    target_fraction = max(
        current_fraction - TRAILING_REDUCE_STEP_FRACTION,
        EXECUTION_MIN_REMAINING_FRACTION,
    )
...
def _step_reduce_exposure_time(ot, symbol, cur_price, ts):
    ...
    for level in REDUCE_EXPOSURE_TIME_LEVELS:
        ...
        ok = _execution_reduce_to_fraction(
            ...
            symbol,
            cur_price,
            level["reduce_to_fraction"],
            tag=key,
            note=f"Reduce Exposure — {level['after_min']} мин в сделке",
        )
        if ok:
            levels_done.add(key)
...
def _step_position_health(ot, symbol, cur_price, ts):
    ...
    log.info(
        f"[{symbol}] POSITION_HEALTH age={age_min}m pnl={pnl_pct} peak={max_pnl} remaining={remaining_pct}"
    )
    send_tg(
        f"📊 <b>{esc(ot.get('name', symbol))} ({esc(symbol)})</b> — health-check\n"
...
def process_execution_policy(ot, symbol, cur_price, ts):
    ...
    if ENABLE_PARTIAL_BINGX:
        if _step_partial_take_profit(ot, symbol, cur_price):
            changed = True
    if ENABLE_BREAK_EVEN:
        if _step_break_even(ot, symbol, cur_price):
            changed = True
    if ENABLE_TRAILING:
        if _step_trailing_stop(ot, symbol, cur_price):
            changed = True
    if ENABLE_REDUCE_EXPOSURE_TIME:
        if _step_reduce_exposure_time(ot, symbol, cur_price, ts):
            changed = True
...
def reconcile_exchange(wl_all, ts, price_full, existing_trade_ids, lifecycle_state):
    ...
    for m in missing:
        ...
        log.error(
            f"RECONCILE MISSING {sym} — на бирже позиции нет, закрываем журнальную сделку"
        )
        close_trade(
            ot,
            sym,
            ...
            exit_candidates=["EXCHANGE_CLOSED"],
            lifecycle_complete=False,
            exchange_close_status="confirmed",
        )  # список позиций уже подтвердил отсутствие на бирже
        ...
        cd = COOLDOWN_BY_EXIT_REASON.get(
            "EXCHANGE_CLOSED", 60
        )  # нет явного маппинга — по умолчанию как DISTRIBUTION/INVALIDATED-класс
        lrec = lifecycle_state.setdefault(sym, {})
        lrec["cooldown_until"] = ts + cd * 60
        lrec["last_exit_reason"] = "EXCHANGE_CLOSED"
...
def run():
    ...
    for sym in list(wl_all.keys()):
        ...
        if not ot or "pending_close" in ot:
            continue  # уже обработано выше в этом прогоне
        ot = dict(ot)
        sig = signals.get(sym)
    ...
    for sym, sig in signals.items():
                ...
                if ENABLE_BINGX and new_ot is not None:
                    try:
                        ...
                        if not bingx_client.contract_exists(sym):
                            ...
                            log.info(
                                f"[{sym}] BingX SKIP: контракт {bx_symbol} не найден"
                            )
                        else:
                            ...
                            if bx.get("status") in ("opened", "already_open"):
                                ...
                                if bx.get("status") == "already_open":
                                    ...
                                    log.error(
                                        f"[{sym}] BingX ALREADY_OPEN при входе — qty_initial "
                                ...
                                log.info(
                                    f"[{sym}] BingX OPEN {bx.get('status')} orderId={bx.get('order_id')} qty={qty_opened}"
                                )
                                log_execution_event(
                                    sym,
                                    "open_confirmed",
                                    status=bx.get("status"),
                                    qty=qty_opened,
                                    order_id=bx.get("order_id"),
                                    qty_initial_uncertain=bx.get(
                                        "qty_initial_uncertain", False
                                    ),
                                )
                            else:
                                log.error(
                                    f"[{sym}] BingX OPEN failed: {bx.get('error')}"
                                )
                            new_ot["bingx"] = bx
                    except Exception as e:
        ...
        if state != prev_state:
            ...
            if state in TG_STATES:
                ...
                msg = format_signal(
                    sym, wl_all[sym], cur, hist, sig["reasons"], sig["warnings"], market
                )
                if llm_res:
