...
# ---- Секреты берём из переменных окружения (GitHub Secrets) ----
COOKIES = {
    "p_sid": os.environ["COINALYZE_P_SID"],
    "chat_sid": os.environ["COINALYZE_CHAT_SID"],
}
...
# ---- Скоринг и режим (те же функции, что раньше) ----
def score_profile_a(r):
    ...
    if r["oi_mktcap_ratio"] is not None and r["oi_mktcap_ratio"] < 0.15:
        score += 1
    if r["oi_vol_ratio"] is not None and 0.1 <= r["oi_vol_ratio"] <= 2.5:
        score += 1
    ls = r["ls_accounts"]
    if ls is not None:
...
def score_profile_b(r):
    ...
    if cvd is not None:
        if cvd < 35: score += 1
        elif cvd > 50: score += 1; flags.append("возможное скрытое накопление на дне")
    return True, score, flags
...
# ---- Работа с JSONL-логом вместо SQLite ----
def load_history():
